...
public class VisualStudioTabControl : TabControl
{
    ...
    public Color DesactiveColor
    {
        get
        {
            return desactiveColor;
        }

        set
        {
            desactiveColor = value;
        }
    }
    ...
    public Color ClosingButtonColorMouseUp
    {
        get
        {
            return closingButtonColorMouseUp;
        }

        set
        {
            closingButtonColorMouseUp = value;
        }
    }
    ...
    protected override void OnMouseDown(MouseEventArgs e)
    {
        ...
        if (predraggedTab != null)
        {
            ...
            if (ShowClosingMessage && showClosingButton)
            {
    ...
    protected override void OnPaint(PaintEventArgs e)
    {
        ...
        for (var i = 0; i < TabCount; i++)
        {
            ...
            {
                Brush ClosingColorBrush = new SolidBrush(closingButtonColor);
            ...
            Drawer.FillRectangle(new SolidBrush(headerColor), Header);

            if (i == SelectedIndex)
            {
                // Drawd tabPage
                DrawTabPage(activeColor, Header, TabPages[i], Drawer);

                // Draws the line top of the color when it is selected
                Drawer.DrawLine(new Pen(horizLineColor, 4.5f), new Point(Header.X, Header.Y), new Point(Header.X + Header.Width, Header.Y));
                ...
                if (showClosingButton)
                {
                    ...
                    {
                        DrawButton(activeColor, ButtonRect, Header, Drawer);
                    }
                }
                else
                {
            ...
            {
                // Drawd tabPage
                DrawTabPage(desactiveColor, Header, TabPages[i], Drawer);
            }

            // Draws the horizontal line
            Drawer.DrawLine(new Pen(horizLineColor, 5), new Point(0, ItemSize.Height + 4), new Point(Width, ItemSize.Height + 4));

            // Draws the background of the tab control
            Drawer.FillRectangle(new SolidBrush(backTabColor), new Rectangle(0, ItemSize.Height + 4, Width, Height - ItemSize.Height));
